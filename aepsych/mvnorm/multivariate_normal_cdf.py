...
from torch import broadcast_to, diagonal, erfc, eye, tril
...
def broadcast_shape(a, b):
    res = reversed(
        tuple(
            i if j == 1 else (j if i == 1 else (i if i == j else -1))
            for i, j in zip_longest(reversed(a), reversed(b), fillvalue=1)
        )
    )
    return list(res)
...
def multivariate_normal_cdf(
    value, loc=0.0, covariance_matrix=None, diagonality_tolerance=0.0
):
    ...
    .. [2] Sébastien Marmin, Clémeant Chevalier and David Ginsbourger, "Differentiating the multipoint Expected Improvement for optimal batch design", International Workshop on Machine learning, Optimization and big Data, Taormina, Italy, 2015. `PDF <https://hal.archives-ouvertes.fr/hal-01133220v4/document>`_.
    Examples
    --------
    >>> import torch
    ...
    if len(cov_shape) < 2:
        raise ValueError(
            "covariance_matrix must have at last " "two dimensions when not diagonal."
        )
    if cov_shape[-2] != d or cov_shape[-1] != d:
        raise ValueError(
            "Covariance matrix must have the last two "
